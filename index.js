...
function StudentMarksEntry() {
    ...
    const handleSave = () => {
        ...
        if (allFieldsFilled) {
            ...
            // Convert data to CSV
            const headers = [
                'Sno', 'Student Name', 'Pen Number', 
                ...['telugu', 'hindi', 'english', 'mathematics', 'evs'].flatMap(subject => 
                    ['FA', 'Speaking', 'Basic Knowledge', 'Writing', 'Corrections', 'Behaviour', 'Activity', 'Total', 'Grade', 'SGPA']
                ),
                'Grand Total', 'Total Grade', 'GPA', 'Percentage'
            ];

            const rows = students.map(student => [
                student.sno, student.studentName, student.penNumber,
                ...['telugu', 'hindi', 'english', 'mathematics', 'evs'].flatMap(subject => [
                    ...student[subject].slice(0, 10)
                ]),
            ...
            alert('Enter the data in required fields');
        }
    };
